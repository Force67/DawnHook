...
namespace hook
{
	...
	public:
		...
		inline pattern& count(uint32_t expected) &
		{
            ...
            if (m_matches.size() != expected) printf("Unable to match\n");
			return *this;
		}
		...
		{
			EnsureMatches(expected);
            //printf("MATCHES : %lld %s\n", m_matches.size(), m_bytes.c_str());
		...
		inline auto first(ptrdiff_t offset = 0)
		{
	...
	{
		return pattern(pattern_string).first<T>(offset);
	}
}
